...
import { Loader2, AlertCircle, X, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="relative flex flex-col items-center w-full">
      {networkGuard.guardSeverity === "error" && (
        <div className="mb-2 w-full max-w-xs bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs pl-3 pr-2 py-2 rounded-lg flex items-start gap-1">
          <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
          <span className="flex-1">{networkGuard.guardMessage}</span>
        </div>
      )}
